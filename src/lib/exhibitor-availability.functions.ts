...
export const listExhibitorAvailability = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<ExhibitorAvailabilityResult> => {
    ...
    // Regra "1 slot = 1 empresa" — coleta pares (table_id, slot_id)
    // fisicamente ocupados. Fonte única do critério "livre vs ocupado":
    // slotIsPhysicallyBooked(meetingsOnPair) em scheduling-rules.
    const pairMeetings = new Map<string, MeetingLite[]>();
    for (const m of meetings) {
      const k = `${m.table_id}::${m.slot_id}`;
      const arr = pairMeetings.get(k) ?? [];
      arr.push({
        id: m.id,
        table_id: m.table_id,
        slot_id: m.slot_id,
        ...
        visitor_company_id:
          visitorById.get(m.visitor_profile_id)?.companyId ?? null,
        start_at: "",
        end_at: "",
      });
      pairMeetings.set(k, arr);
    }
    const bookedSlotsPerTable = new Map<string, Set<string>>();
    for (const [k, arr] of pairMeetings.entries()) {
      if (!slotIsPhysicallyBooked(arr)) continue;
      const [tid, sid] = k.split("::");
      const set = bookedSlotsPerTable.get(tid) ?? new Set<string>();
      set.add(sid);
      bookedSlotsPerTable.set(tid, set);
    }
    ...
    for (const t of tableList) {
      ...
      for (const s of tableSlots) {
        // Livre = par (table_id, slot_id) sem NENHUMA meeting scheduled.
        // Idêntico ao critério de scheduling-rules.slotIsPhysicallyBooked.
        if (bookedSlotsPerTable.get(t.id)?.has(s.id)) continue;
        acc.free_slots.push({
          slot_id: s.id,
          table_id: t.id,
