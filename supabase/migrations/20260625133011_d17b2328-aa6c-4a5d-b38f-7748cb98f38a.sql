...
BEGIN
  ...
  -- Try to reuse pre-registered company by tax_id (digits-only) when the
  -- current profile does not already point at one.
  IF v_existing_company IS NULL AND v_tax_id IS NOT NULL THEN
    ...
    IF length(coalesce(v_tax_digits, '')) > 0 THEN
      ...
      IF v_found_company IS NOT NULL THEN
        -- Defense in depth: never silently link a CNPJ that already has a
        -- claimed account. The frontend pre-check normally blocks this,
        -- but the RPC must enforce it too.
        SELECT EXISTS (
        ...
        -- Inspect pending profiles linked to this company (excluding self)
        -- to decide whether to flag email mismatch for review.
        SELECT
          COUNT(*) > 0,
  ...
  -- Merge review reasons: preserve existing ones, add quality + new mismatch.
