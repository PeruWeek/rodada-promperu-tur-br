-- ─────────────────────────────────────────────────────────────────────────────
-- Fonte única da verdade para "cadastro concluído" do visitante/comprador.
--
-- Esta migration:
--   1) Endurece a RPC `complete_buyer_signup` para rejeitar payloads sem
--      qualquer campo obrigatório (CNPJ/tax_id, cidade, UF, cargo, whatsapp
--      e idioma), além dos já enforçados (almoço, autorização de imagem,
--      consentimento, trade_name, full_name).
--   2) Expande o trigger `enforce_visitor_signup_completion_fields` para
--      validar TODOS os obrigatórios da regra central via JOIN com
--      profiles + companies sempre que `signup_completed_at` for marcado.
--      Garante que importadores, upserts diretos, /profile ou staff dialog
--      não consigam marcar como completo sem todos os obrigatórios.
--   3) Stubs/importados continuam permitidos como PENDENTES (sem
--      signup_completed_at) — pré-cadastro continua viável.
--
-- Regra final usada no banco: visitor_profiles.signup_completed_at só pode
-- ser gravado quando o trigger validar TODOS os campos abaixo.
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- ── Obrigatórios duros (regra central) ────────────────────────────────────
  ...
  IF v_trade_name IS NULL THEN RAISE EXCEPTION 'trade_name_required'; END IF;
  IF v_tax_id IS NULL THEN RAISE EXCEPTION 'tax_id_required'; END IF;
  IF v_city IS NULL THEN RAISE EXCEPTION 'city_required'; END IF;
  IF v_state IS NULL THEN RAISE EXCEPTION 'state_code_required'; END IF;
  IF v_full_name IS NULL THEN RAISE EXCEPTION 'full_name_required'; END IF;
  IF v_job_title IS NULL THEN RAISE EXCEPTION 'job_title_required'; END IF;
  IF v_whatsapp IS NULL THEN RAISE EXCEPTION 'whatsapp_required'; END IF;
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Trigger expandido: ao marcar signup_completed_at, valida TODOS os
-- obrigatórios via JOIN com profiles + companies. Mesma regra central que
-- vive em src/lib/registration-requirements.ts.
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  IF NEW.signup_completed_at IS NOT NULL
    ...
    END IF;
    IF v_profile.preferred_language IS NULL THEN
    ...
    END IF;
    IF coalesce(btrim(v_company.tax_id), '') = '' THEN
...
-- Trigger já existe (criado em 20260625235022); função recriada acima.
