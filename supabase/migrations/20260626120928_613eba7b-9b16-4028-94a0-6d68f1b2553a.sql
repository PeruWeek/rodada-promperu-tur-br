-- Permitir múltiplos contatos (profiles) na mesma empresa, compartilhando o
-- mesmo CNPJ/company_id. Cada pessoa mantém login/e-mail próprio e agenda
-- individual (meetings já são vinculadas a visitor_profile_id, não à
-- empresa, então o modelo já comporta agendas separadas por contato).
--
-- Mudança: remover o bloqueio `cnpj_already_claimed` em
-- complete_buyer_signup. Quando o CNPJ já existir e tiver pelo menos um
-- usuário ativo, o novo cadastro passa a SE VINCULAR à empresa existente
-- como contato adicional, em vez de falhar.
...
BEGIN
  ...
  -- Reutiliza empresa existente pelo CNPJ — mesmo que já tenha outros
  -- usuários ativos. Múltiplos contatos por empresa são permitidos: cada
  -- pessoa fica com seu próprio profile/login e agenda individual.
  IF v_existing_company IS NULL AND v_tax_id IS NOT NULL THEN
    ...
    IF length(coalesce(v_tax_digits, '')) > 0 THEN
      ...
      IF v_found_company IS NOT NULL THEN
        ...
        -- Atualiza dados da empresa de forma não-destrutiva: mantém valores
        -- existentes quando o novo contato não informar algo diferente.
        UPDATE public.companies
        SET trade_name = COALESCE(v_trade_name, trade_name),
